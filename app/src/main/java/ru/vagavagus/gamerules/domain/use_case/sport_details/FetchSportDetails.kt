package ru.vagavagus.gamerules.domain.use_case.sport_details
...
class FetchSportDetails(
    private val repository: SportRepository
) {
    private val tag = this::class.simpleName

    operator fun invoke(language: String, reference: String) = flow {
        try {
            emit(RequestResult.Loading())
            val data = repository.fetchSportDetailsByReference(language, reference)
